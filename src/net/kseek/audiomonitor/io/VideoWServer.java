package net.kseek.audiomonitor.io;
...
import net.kseek.audiomonitor.data.ImageBufferManager;
import net.kseek.audiomonitor.data.DataListener;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.google.gson.JsonParseException;
import com.google.gson.JsonParser;

public class VideoWServer extends Thread {
	private ServerSocket server;
	private DataListener dataListener;
	private ImageBufferManager imageBufferManager;
	private int socketPort;

	public VideoWServer(int port) {
		socketPort = port;
	}
			...
			while (!Thread.currentThread().isInterrupted()) {
				...
				while ((len = inputStream.read(buff)) != -1) {
	                ...
	                if (isJSON && element != null) {
	                    ...
	                    if (element != null && element.getAsString().equals("data")) {
                            ...
                            imageBufferManager = new ImageBufferManager(length, width, height);
                            imageBufferManager.setOnDataListener(dataListener);
                            break;
	                    }
	                }
	                else {
				...
				if (imageBuff != null) {
				    ...
				    while ((len = inputStream.read(imageBuff)) != -1) {
	                    imageBufferManager.fillBuffer(imageBuff, len);
	                }
				}
				
				if (imageBufferManager != null) {
					imageBufferManager.close();
				}
			}
		} catch (IOException e) {
